...
function Physics(canvasWidth, canvasHeight, context) {
	...
	/*this.createBox(canvasWidth, 1, canvasWidth/2, canvasHeight, this.b2Body.b2_staticBody); //bottom
	this.createBox(canvasWidth, 1, canvasWidth/2, 0, this.b2Body.b2_staticBody); //top
	this.createBox(1, canvasHeight, 0, canvasHeight/2, this.b2Body.b2_staticBody); //left
	this.createBox(1, canvasHeight, canvasWidth, canvasHeight/2, this.b2Body.b2_staticBody); //right*/
...
Physics.prototype.createBox = function(width, height, pX, pY, type) {
...
Physics.prototype.update = function() {
	...
	for (var b = this.world.m_bodyList; b !== null; b = b.m_next) {
		...
		if( b.GetType() !== this.b2Body.b2_staticBody) {
			...
			var accel = Math.max( 0, (distance - 10) / 1000);
			...
			b.ApplyForce( new this.b2Vec2(xVel * 20, yVel * 20), new this.b2Vec2(xPos, yPos));

			for( var f = b.m_fixtureList; f; f = f.m_next ) {
				f.GetShape().m_radius = (Math.floor(Math.random() * 10) + 1) / this.worldScale;
			}
		}
		/*else {
			...
			if (b === this.whiteCircle) {
				...
				for( var f = b.m_fixtureList; f; f = f.m_next ) {
					...
					var s = f.GetShape();
					f.GetShape().m_radius = f.GetShape().m_radius + (this.previousValue * 10) / this.worldScale;
					this.whiteCircle.SetPosition(new this.b2Vec2( this.whiteCircle.GetPosition().x - (this.previousValue * -10) / this.worldScale, this.whiteCircle.GetPosition().y - (this.previousValue * -10) / this.worldScale ));
					//this.world.DrawShape(s, xf, color);

					// Paint the node in the canvas
					this.context.fillStyle = '#fff';
					this.context.beginPath();
					this.context.arc(this.whiteCircle.GetPosition().x * this.worldScale, this.whiteCircle.GetPosition().y * this.worldScale, f.GetShape().m_radius * this.worldScale, 0, 2 * Math.PI, false);
